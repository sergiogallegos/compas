...
/// Shared-grain WSOLA stretcher for **key-locked stems**.
///
/// Stretching each separated stem with its own independent [`TimeStretch`] lets the per-stem
/// similarity searches pick *different* grain offsets at each hop, so a transient in one stem (a
/// kick in `drums`) can drift out of phase with the matching event in another (`bass`) — audible
/// smearing when the stems are summed back. This stretcher places every grain **once**, from a
/// single similarity search on a shared reference signal (the deck's full mix), then overlap-adds
/// that same grain position into all `N` stems. Because overlap-add is linear and shares the
/// window and position, `Σᵢ OLA(stemᵢ, pos) = OLA(Σᵢ stemᵢ, pos)`, so the summed stems reproduce
/// the coherently-stretched mix and inter-stem transients stay aligned. It is also cheaper than
/// `N` independent stretchers — the O(search·hop) cost is paid once, not `N` times.
///
/// All buffers are pre-allocated; per-sample processing is allocation-free and RT-SAFE (one
/// similarity search per hop, shared across the stems).
pub struct StemStretch<const N: usize> {
    win: Vec<f32>,
    /// Per-stem synthesis accumulators (overlap-add output), each `STRETCH_WINDOW` long.
    accum_l: [Vec<f32>; N],
    accum_r: [Vec<f32>; N],
    /// Shared similarity-search scratch (computed on the reference, reused for all stems).
    cand_mono: Vec<f32>,
    template: Vec<f32>,
    emit_idx: usize,
    prev_pos: f64,
...
impl<const N: usize> StemStretch<N> {
    pub fn new() -> Self {
        let n = STRETCH_WINDOW;
        let win = (0..n)
            .map(|k| 0.5 * (1.0 - (2.0 * PI * k as f32 / n as f32).cos()))
        ...
        StemStretch {
            win,
            accum_l: std::array::from_fn(|_| vec![0.0; n]),
            accum_r: std::array::from_fn(|_| vec![0.0; n]),
            cand_mono: vec![0.0; 2 * STRETCH_SEARCH + STRETCH_HOP],
            template: vec![0.0; STRETCH_HOP],
            emit_idx: 0,
            prev_pos: 0.0,
    ...
    /// Drop buffered output and re-prime on the next sample (engage / seek / after scratching /
    /// stem swap, where the play-head jumps discontinuously). RT-SAFE.
    pub fn reset(&mut self) {
        self.primed = false;
        self.emit_idx = 0;
    }

    /// Pull one device-rate stereo frame for each of the `N` stems into `out`.
    ///
    /// `ref_samples`/`ref_frames` is the shared search reference (the deck's full mix) used to
    /// place each grain once for all stems; `stems[i]` = `(interleaved-stereo samples, frame
    /// count)` for stem `i`; `base_ratio` = source_rate/device_rate; `anchor` is the current
    /// play-head (source frames), shared by every stem. RT-SAFE.
    #[inline]
    pub fn next_frame(
        &mut self,
        ref_samples: &[f32],
        ...
        if !self.primed {
            self.prime(stems, base_ratio, anchor);
        }
        for (i, slot) in out.iter_mut().enumerate() {
            *slot = (
                self.accum_l[i][self.emit_idx],
                self.accum_r[i][self.emit_idx],
            );
        }
        self.emit_idx += 1;
        if self.emit_idx >= STRETCH_HOP {
            self.advance(ref_samples, ref_frames, stems, base_ratio, anchor);
            self.emit_idx = 0;
        }
    }

    fn prime(&mut self, stems: &[(&[f32], usize); N], base_ratio: f64, anchor: f64) {
        for i in 0..N {
            self.accum_l[i].iter_mut().for_each(|x| *x = 0.0);
            self.accum_r[i].iter_mut().for_each(|x| *x = 0.0);
        }
        self.overlap_add(stems, base_ratio, anchor);
        self.prev_pos = anchor;
        self.emit_idx = 0;
        self.primed = true;
    }
    ...
    fn advance(
        ...
        // Slide each stem's finished hop out; the second half becomes the new overlap region.
        for i in 0..N {
            self.accum_l[i].copy_within(STRETCH_HOP.., 0);
            self.accum_r[i].copy_within(STRETCH_HOP.., 0);
            for k in (STRETCH_WINDOW - STRETCH_HOP)..STRETCH_WINDOW {
                self.accum_l[i][k] = 0.0;
                self.accum_r[i][k] = 0.0;
            }
        }
        // One shared grain decision (searched on the reference), applied to every stem.
        let pos = self.best_grain(ref_samples, ref_frames, base_ratio, anchor);
        self.overlap_add(stems, base_ratio, pos);
        self.prev_pos = pos;
    }

    /// Similarity search on the shared reference: pick the grain near `anchor` whose head best
    /// matches the previous grain's natural continuation, for phase continuity. Identical to
    /// [`TimeStretch::best_grain`] but run once and reused for all stems.
    fn best_grain(
        &mut self,
        ref_samples: &[f32],
        ref_frames: usize,
        ...
        for j in 0..STRETCH_HOP {
            let pos = self.prev_pos + ((STRETCH_HOP + j) as f64) * base_ratio;
            self.template[j] = lerp_mono(ref_samples, ref_frames, pos);
        }
        let span = 2 * STRETCH_SEARCH + STRETCH_HOP;
        for i in 0..span {
            let pos = anchor + ((i as f64) - STRETCH_SEARCH as f64) * base_ratio;
            self.cand_mono[i] = lerp_mono(ref_samples, ref_frames, pos);
        }
        let mut best_o = STRETCH_SEARCH; // offset 0 (= anchor) by default
        let mut best = f32::NEG_INFINITY;
        for o in 0..=(2 * STRETCH_SEARCH) {
    ...
    fn overlap_add(&mut self, stems: &[(&[f32], usize); N], base_ratio: f64, pos: f64) {
        for k in 0..STRETCH_WINDOW {
            let w = self.win[k];
            let sp = pos + (k as f64) * base_ratio;
            for (i, &(samples, frames)) in stems.iter().enumerate() {
                self.accum_l[i][k] += w * lerp_ch(samples, frames, sp, 0);
                self.accum_r[i][k] += w * lerp_ch(samples, frames, sp, 1);
            }
        }
    }
}

impl<const N: usize> Default for StemStretch<N> {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    #[test]
    fn stem_stretch_shares_grains_so_summed_stems_equal_the_stretched_mix() {
        // Two decorrelated tones as "stems"; their exact sum is the mix.
        let frames = 48_000usize;
        let a = sine_src(436.0, frames); // ~110 Hz @ 48 kHz (bass-like)
        let b = sine_src(173.0, frames); // ~277 Hz (a different, unrelated period)
        let mut mix = vec![0.0f32; frames * 2];
        for i in 0..frames * 2 {
            mix[i] = a[i] + b[i];
        }
        let zero = vec![0.0f32; frames * 2];

        let base_ratio = 1.0;
        let tempo = 1.06f64; // key-locked +6%
        let n = 16_000usize;

        let mut mix_st = TimeStretch::new(); // reference: single-stream mix stretch
        let mut shared = StemStretch::<4>::new();
        // Teeth: the OLD per-stem approach — one independent stretcher per stem.
        let mut ind_a = TimeStretch::new();
        let mut ind_b = TimeStretch::new();

        let mut anchor = 0.0f64;
        let mut shared_err = 0.0f32;
        let mut indep_err = 0.0f32;
        for _ in 0..n {
            let (ml, _) = mix_st.next_frame(&mix, frames, base_ratio, anchor);

            let stem_refs: [(&[f32], usize); 4] = [
                (&a[..], frames),
                (&b[..], frames),
                (&zero[..], frames),
                (&zero[..], frames),
            ...
            shared.next_frame(&mix, frames, &stem_refs, base_ratio, anchor, &mut out);
            let shared_sum = out[0].0 + out[1].0 + out[2].0 + out[3].0;
            shared_err = shared_err.max((shared_sum - ml).abs());

            let (al, _) = ind_a.next_frame(&a, frames, base_ratio, anchor);
            let (bl, _) = ind_b.next_frame(&b, frames, base_ratio, anchor);
            indep_err = indep_err.max((al + bl - ml).abs());

            anchor += tempo;
        }

        // Shared grains: summing the stems reconstructs the coherently-stretched mix (linearity
        // of overlap-add under a single, shared grain schedule) — to fp tolerance.
        assert!(
            shared_err < 1e-3,
            "shared-grain stems must reconstruct the stretched mix: err={shared_err}"
        );
        // Independent per-stem stretching picks different grain offsets per stem, so the stems no
        // longer line up — the very incoherence this stretcher fixes.
        assert!(
            indep_err > 10.0 * shared_err.max(1e-4),
            "independent per-stem stretching should lose coherence vs shared grains: \
             indep={indep_err}, shared={shared_err}"
        );
    }

    #[test]
    fn stem_stretch_reset_reprimes_and_output_is_bounded() {
        let frames = 20_000usize;
        let a = sine_src(73.0, frames);
        let b = sine_src(131.0, frames);
        let zero = vec![0.0f32; frames * 2];
        let mut st = StemStretch::<4>::new();
        let stem_refs: [(&[f32], usize); 4] = [
            (&a[..], frames),
            (&b[..], frames),
            (&zero[..], frames),
        ...
        for k in 0..10_000 {
            if k == 5_000 {
                st.reset(); // mid-run seek: must re-prime cleanly, no NaN/inf
                anchor = 0.0;
            }
            st.next_frame(&mix, frames, &stem_refs, 1.0, anchor, &mut out);
            for (l, r) in out.iter() {
                assert!(l.is_finite() && r.is_finite(), "non-finite stem output");
                peak = peak.max(l.abs());
            }
            anchor += 1.05;
        }
        assert!(peak > 0.1, "stem output suspiciously quiet (peak {peak})");
        assert!(peak < 2.0, "stem output overshoots badly (peak {peak})");
    }
}
