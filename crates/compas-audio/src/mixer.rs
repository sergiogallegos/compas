...
/// RT-side parking capacity for retired buffers when the reclaim ring is briefly full. Sized to
/// absorb every deck swapping its mix buffer at once, with headroom.
...
/// Key-lock (master-tempo) stage: per-deck WSOLA time-stretch for the mix buffer.
/// `active` mirrors the deck's key-lock toggle; `engaged` tracks the previous frame's read mode so a
/// play-head jump (seek / loop / scratch release / toggle) re-primes the stretcher before the next
/// stretched frame. All stretch buffers are allocated on construction; nothing here allocates,
...
impl DeckPlayer {
    ...
    fn next_frame(&mut self) -> (f32, f32) {
        ...
        // Source-read stage, then play-head advance: sample the mix buffer at the current play-head
        // (through the key-lock stretcher when engaged), then move the play-head for the next frame
        // (scratch / sync / tempo, loop-roll slip, beat-loop wrap).
    ...
    /// Source-read stage: sample the mix buffer at the current play-head and return one stereo
    /// frame; `engaged` selects the key-lock stretcher over direct interpolation. The play-head is
